...
class AuthController
{
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_POST['data']) && !empty($_POST['data'])) {
                ...
                if ($usuario->validar()) {
                    ...
                    $stmt = $db->prepare('UPDATE pacientes SET token = :token WHERE correo = :correo');
                    $stmt->bindParam(':token', $token);
                    $stmt->bindParam(':correo', $usuario->getEmail());
                    $stmt->execute();
    ...
    public function processLogin()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $db = new \PDO('mysql:host=localhost;dbname=clinica', 'root', '');
            $stmt = $db->prepare('SELECT * FROM pacientes WHERE correo = :correo LIMIT 1');
